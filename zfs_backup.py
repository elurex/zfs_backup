...
for a in pools:
    i=a.split(",")[0]
    r=a.split(",")[1]
    getsnapshots = subprocess.Popen("zfs list -t snapshot {}".format(i),shell=True, stdout=subprocess.PIPE )
    ...
    if(count > 1):
        checkremote = subprocess.Popen("ssh {} 'zfs list -t snapshot | grep {}@{}'".format(remote_address,r,snapshot_stamp),shell=True, stdout=subprocess.PIPE )

        if(i == checkremote.communicate()[0].decode("utf-8").strip().split('@')[0]):
            #send incremental snapshot
            subprocess.run("zfs send -i {} {}@{} | ssh {} zfs recv {}".format(previous_snapshot,i,today,remote_address,r),shell=True)
            #remove old snapshot
            if(count - retention > 0):
                for n in range(count - retention):
                    subprocess.run("zfs destroy {}".format(snapshots[n].split(" ")[0]),shell=True)
                    subprocess.run("ssh {} zfs destroy {}".format(remote_address,snapshots[n].replace(i,r).split(" ")[0]),shell=True)
        else:
             #send complete snapshot since there is no increments
             subprocess.run("zfs send {}@{} | ssh {} zfs recv {} -F".format(i,today,remote_address,r),shell=True)
    else:
        #send complete snapshot since there is no increments
        subprocess.run("zfs send {}@{} | ssh {} zfs recv {} -F".format(i,today,remote_address,r),shell=True)
